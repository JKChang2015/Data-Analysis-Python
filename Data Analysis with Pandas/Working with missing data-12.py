...
age_null = pd.isnull(titanic_survival["age"])
vector = titanic_survival["age"]
vector = vector[age_null == False]
...
for pc in passenger_classes:
    condition = titanic_survival["pclass"] == pc
    fare = titanic_survival[condition]["fare"]
    fares_by_class[pc] = fare.mean()
...
print(passenger_survival)
port_stats = titanic_survival.pivot_table(index = "embarked",values = ["age","survived","fare"])
print(port_stats)
