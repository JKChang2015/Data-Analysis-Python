...
auc1 = roc_auc_score(predictions1, test["high_income"])
auc2 = roc_auc_score(predictions2,test["high_income"])
...
print(roc_auc_score(final, test["high_income"]))
...
final_pred = numpy.round(sum(predictions)/len(predictions))

auc = roc_auc_score(final_pred, test["high_income"])
print(auc)
