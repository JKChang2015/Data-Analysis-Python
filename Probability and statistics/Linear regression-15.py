...
slope_density = cov(wine_quality["density"].values,wine_quality["quality"].values)[0,1] / wine_quality["density"].var()
...
intercept_density = y_mean - calc_slope(wine_quality["density"], wine_quality["quality"]) * x_mean
...
m = calc_slope(x,y)  
c = calc_intercept(x,y,m)

def predict(x):
    return m * x + c

predicted_quality = x.apply(predict)
...
predicted_values = slope * wine_quality["density"] + intercept
rss = np.sum((wine_quality["quality"] - predicted_values) ** 2)
