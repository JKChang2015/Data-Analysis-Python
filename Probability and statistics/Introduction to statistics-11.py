...
sr = ["none","a few","some","a lot"]
p = [sr.index(r) for r in survey_responses ]
average_smoking = sum(p)/len(p)
...
ms= [savings[idx] for idx,g in enumerate(gender) if g == "male"]
fs = [savings[idx] for idx,g in enumerate(gender) if g == "female"]
male_savings = sum(ms)/len(ms)
female_savings = sum(fs)/len(fs)
...
med = numpy.median(test_scores_positive)
plt.hist(test_scores_positive)
plt.axvline(median, color = 'b')
...
plt.axvline(new_titanic_survival.mean(), color = 'red')
plt.axvline(new_titanic_survival.median(), color = 'blue')
plt.show()
